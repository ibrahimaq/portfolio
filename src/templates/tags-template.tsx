...
const TagsPage = () => {
  ...
  data?.allContentfulBlog?.nodes?.forEach(
    (node: {
      markdown: { childMarkdownRemark: { frontmatter: { tags: string[] } } }
    }) => {
    ...
    <Layout>
      ...
      <Container className="pt-[150px]">
        <h1 className="text-center text-base md:text-lg">
          ...
          <span className="block w-fit mx-auto text-2xl md:text-3xl px-3 py-1 mt-2 rounded-lg">
            #{camelCase(activeTag)}
          </span>
        </h1>
