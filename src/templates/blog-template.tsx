...
const BlogTemplate = ({ data }: IBlogTemplate) => {
  ...
  useEffect(() => {
    ...
    setFilteredBlogs(
      removeSameBlog(relatedBlogs as Queries.ContentfulBlog[], slug)
    )
  }, [])
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="flex items-center my-5">
                <Book
                  fill="#545456"
                  width="20px"
                  customClass="inline-block mr-2"
                />
                <p>
                  {blog?.markdown?.childMarkdownRemark?.timeToRead} min read
                </p>
              </div>
              <TagsList
                tags={
                  blog?.markdown?.childMarkdownRemark?.frontmatter
                    ?.tags as string[]
                }
              />
            </div>
      ...
      {filteredBlogs && filteredBlogs.length > 0 ? (
        <section className="pt-20">
          <Container>
            <div className="mb-5">
              <SectionTitle
                title={`Related ${filteredBlogs.length > 1 ? "blogs" : "blog"}`}
              />
            </div>
...
export const query = graphql`
  query singleBlog($slug: String!, $relatedBlogsByTags: [String!]!) {
    ...
    relatedBlogs: allContentfulBlog(
      limit: 3
      filter: {
        childrenContentfulBlogMarkdownTextNode: {
          elemMatch: {
            childrenMarkdownRemark: {
              elemMatch: { frontmatter: { tags: { in: $relatedBlogsByTags } } }
            }
          }
        }
      }
