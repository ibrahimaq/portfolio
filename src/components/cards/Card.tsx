  ...
  return (
    <>
      {data.markdown && data.id && (
        <div
          key={data.id}
          className="flex flex-col overflow-hidden rounded-md shadow-lg h-full "
        >
          <Link
            to={`/blogs/${data.markdown?.childMarkdownRemark?.frontmatter?.slug}`}
          ...
          >
            ...
            <p className="text-sm absolute bottom-0 bg-white left-0 px-4 py-2">
              {data.date}
            </p>
          </Link>
          ...
          <div className="p-5 relative flex flex-col h-full">
            ...
            <div className="mt-auto">
              ...
              <ul className="flex flex-row flex-wrap">
                {data?.markdown?.childMarkdownRemark?.frontmatter?.tags
                  ?.slice(0, 3)
                  .map((tag, i) => {
                    return (
                      <li key={i} className="mr-2">
                        <Link
                          to={`/tags/${kebabCase(tag ? tag : undefined)}`}
