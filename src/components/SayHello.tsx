    ...
    <section>
      <Container className="py-20">
        <div className="w-full flex flex-col items-center bg-white rounded-md mx-auto">
          {heading && (
            <h2 className="text-center sm:text-start text-3xl md:text-4xl text-dark">
              {heading}
            </h2>
          )}
          {body && <p className="text-center my-4 text-dark text-xl">{body}</p>}
          ...
          <div className="flex flex-row justify-center space-x-[10px]">
            ...
            <IconContext.Provider
              value={{
                className: `w-[70px] h-[70px] ${themeClassBuilder({
                  color,
                  el: "text",
                })}`,
              }}
            >
              {socials.map((item, i) => (
                <a
                  key={i}
                  href={item.link}
                  className="shadow-md hover:shadow-2xl hover:scale-105 transition-all duration-300  rounded-full"
                >
                  {item.icon}
                </a>
              ))}
