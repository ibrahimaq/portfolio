...
const Button = ({
  ...
  const getBtnStyle = () => {
    return `relative w-full flex flex-col justify-center items-center border-none rounded-sm shadow-xl cursor-pointer font-medium tracking-wider transition-all duration-200 ease-in-out ${themeClassBuilder(
      { color, el: "bg" }
    )} ${className}
  ...
  const btnStyle = `relative w-full flex flex-col justify-center items-center border-none rounded-sm shadow-xl cursor-pointer font-medium tracking-wider transition-all duration-200 ease-in-out ${themeClassBuilder(
    { color, el: "bg" }
  )} ${className}
		${small && "md:text-lg min-w-[130px] min-h-[45px]"}
		${large && "text-lg md:text-xl min-w-[160px] min-h-[60px]"}
  ...
  return (
    <>
      {link && (
        <Link className={btnStyle} to={link}>
          {label}
        </Link>
      )}
