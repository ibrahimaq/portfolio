...
const ThemeOption = () => {
  ...
  return (
    <>
      <div
        ref={ref}
        className={`z-[100] fixed rounded-full right-2 md:right-5 bottom-[50px] flex flex-col h-max`}
      >
        <IconContext.Provider
          value={{ className: "text-white w-[30px] h-[30px]" }}
        >
          <button
            className={`p-1 ${themeClassBuilder({
              color,
              el: "bg",
            })} shadow-3d rounded-full cursor-pointer text-white`}
