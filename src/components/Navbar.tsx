...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <div
        className={`hidden md:block md:absolute md:w-full md:pt-8 bg-transparent z-10 `}
      >
        <div className="flex flex-row justify-between items-center px-16 max-w-[1440px] mx-auto">
          <div className="w-[40px] h-[40px]">
            <img src={avatar} alt="avatr of Ibrahim" className="object-cover" />
          </div>
      ...
      {/* Mobile */}
      <div
        className={`md:hidden w-full fixed z-[100] transition-all duration-300 
      ...
      >
        ...
        <nav
          className={`${
            mobileMenu ? "scale-100 opacity-100  z-[-1]" : "scale-0 opacity-0"
  ...
  return transitions(
    (style, item) =>
      item && (
        <animated.div
          style={style}
          className="relative px-[24px] mt-[20px] relative flex flex-row justify-center max-w-[300px] mx-auto"
        >
          <Button
            label="Let's chat"
